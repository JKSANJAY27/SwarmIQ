    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 0, 'completed': phase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Create simulation instance, pull simulation world parameter template.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Based on context, automatically extract entities and relations from knowledge graph, initialize simulation agents, and assign unique behaviors and memories based on reality seeds.
          </p>
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">Correlated Topics</span>
            </div>
          </div>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">Configure Global Simulation Settings</span>
          </div>
          <div class="step-status">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            LLM intelligently configures global time flow, recommendation algorithms, active periods, posting frequencies, and event triggers.
          </p>
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                ...
                <div class="config-item">
                  <span class="config-item-label">Ticks / Hr</span>
                  <span class="config-item-value">{{ simulationConfig.time_config?.minutes_per_round || '-' }} mins</span>
                </div>
                <div class="config-item">
                  <span class="config-item-label">Total Ticks</span>
                  <span class="config-item-value">{{ Math.floor((simulationConfig.time_config?.total_simulation_hours * 60 / simulationConfig.time_config?.minutes_per_round)) || '-' }} ticks</span>
                </div>
                <div class="config-item">
            ...
            <div class="config-block">
              <div class="config-block-header">
                <span class="config-block-title">Recommendation Algorithm Settings</span>
              </div>
              <div class="platforms-grid">
                <div v-if="simulationConfig.twitter_config" class="platform-card">
                  ...
                  <div class="platform-params">
                    ...
                    <div class="param-row">
                      <span class="param-label">Echo Chamber Str.</span>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Automatically generate initial activation events and hot topics based on narrative direction, guiding the initial state of the simulation.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <!-- Agent 数量配置 -->
          <div v-if="simulationConfig" class="rounds-config-section agent-config-section" style="margin-bottom: 24px;">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Agent Settings</span>
                <span class="section-desc">Select the total number of artificial agents to simulate in the environment. Note: Requires local LLM processing time ~1 min/agent initially.</span>
              </div>
            </div>
            ...
            <div class="rounds-content custom">
              <div class="slider-display">
                <div class="slider-main-value">
                  <span class="val-num">{{ customNumAgents }}</span>
                  <span class="val-unit">agents</span>
                </div>
              </div>
              ...
              <div class="range-wrapper">
                <input 
                  type="range" 
                  v-model.number="customNumAgents" 
                  min="2" 
                  max="50"
                  step="1"
                  class="minimal-slider"
                  :style="{ '--percent': ((customNumAgents - 2) / 48) * 100 + '%' }"
                />
                <div class="range-marks">
                  <span>2</span>
                  <span 
                    class="mark-recommend" 
                    :class="{ active: customNumAgents === 5 }"
                    @click="customNumAgents = 5"
                    :style="{ position: 'absolute', left: `calc(${(5 - 2) / 48 * 100}% - 30px)` }"
                  >5 (Recommended)</span>
                  <span>50</span>
                </div>
              </div>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">Simulation Tick Settings</span>
                <span class="section-desc">SwarmIQ automatically plans to simulate <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours, each tick represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes of time.</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  <div class="slider-main-value">
                    ...
                    <span class="val-unit">ticks</span>
                  </div>
                  <div class="slider-meta-info">
                    <span>With {{ customNumAgents }} Agents: Estimated Engine Time ~{{ Math.max(1, Math.round(customMaxRounds * customNumAgents * 0.006)) }} mins</span>
                  </div>
                </div>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        With {{ customNumAgents }} Agents: Estimated Engine Time ~{{ Math.max(1, Math.round(autoGeneratedRounds * customNumAgents * 0.006)) }} mins
                      </span>
                    </div>
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">If first run, we highly recommend switching to 'Custom' mode to reduce ticks and preview results safely ➝</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div class="action-group dual">
            ...
            >
              Start Multi-platform Simulation ➝
            </button>
          </div>
        </div>
      </div>
      ...
      <div v-if="selectedProfile" class="profile-modal-overlay" @click.self="selectedProfile = null">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            ...
            <div class="info-item">
              <span class="info-label">MBTI Apparent</span>
          ...
          <div class="modal-section">
            <span class="section-label">Persona Biography</span>
            <p class="section-bio">{{ selectedProfile.bio || 'No biography provided' }}</p>
          </div>

          <!-- 关注话题 -->
          <div class="modal-section" v-if="selectedProfile.interested_topics?.length">
            <span class="section-label">Associated Seed Topics</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              <div class="dimension-card">
                <span class="dim-title">Event Experiences</span>
                <span class="dim-desc">Complete behavioral trajectory in this event</span>
              </div>
              <div class="dimension-card">
                <span class="dim-title">Behavioral Profiling</span>
...
const customNumAgents = ref(5)    // 默认推荐5个Agents
const expectedTotal = computed(() => customNumAgents.value)
...
const handleStartSimulation = () => {
  // 构建传递给父组件的参数
  const params = {
    numAgents: customNumAgents.value
  }
  
  if (useCustomRounds.value) {
    // 用户自定义轮数，传递 max_rounds 参数
    params.maxRounds = customMaxRounds.value
    addLog(`Starting simulation, custom rounds: ${customMaxRounds.value} ticks, ${customNumAgents.value} agents`)
  } else {
    // 用户选择保持自动生成的轮数，不传递 max_rounds 参数
    addLog(`Starting simulation, auto-generated rounds: ${autoGeneratedRounds.value} ticks, ${customNumAgents.value} agents`)
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
        ...
        addLog(`✗ Setup failed: ${data.error || '未知错误'}`)
        stopPolling()
        stopProfilesPolling()
      }
    }
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          ...
          addLog(`  ├─ Sim duration: ${data.summary.simulation_hours}hours`)
        ...
        if (data.config.time_config) {
          ...
          addLog(`Time config: Every${tc.minutes_per_round}minutes, Total${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)}轮`)
        }
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        if (res.data.summary) {
          ...
          addLog(`  ├─ Sim duration: ${res.data.summary.simulation_hours}hours`)
        ...
        // 配置尚未生成，开始轮询
        addLog('Generating config, polling for completion...')
        startConfigPolling()
      }
    }
  } catch (err) {
